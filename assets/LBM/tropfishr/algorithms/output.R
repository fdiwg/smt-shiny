...
textTropFishR.diag1 <- function(elefan_ga, input){

    file.name <- elefan_ga$dataExplo$checks$fileName

    ## input data format
    format <- "not clear"
    if(elefan_ga$dataExplo$checks$format == "wide") {
        format <- "length frequency table"
    }
    if(elefan_ga$dataExplo$checks$format == "long") {
        format <- "raw length measurements"
    }

    ## Number of length bins
    l.diff <- diff(sort(unique(elefan_ga$dataExplo$raw$Length)))
    l.diff.min <- min(l.diff)
    l.diff.max <- max(l.diff)

    ## Range of lengths
    l.min <- min(elefan_ga$dataExplo$raw$Length)
    l.max <- max(elefan_ga$dataExplo$raw$Length)

    ## Number of time points (samples)
    samp.times <- unique(elefan_ga$dataExplo$raw$Date)
