...
The uploaded data set (\verb|`r input$file$name`|) contains a catch time series
spanning from `r min(spict_dat$dataExplo$inp$timeC)` to `r max(spict_dat$dataExplo$inp$timeC)` and `r ifelse(spict_dat$dataExplo$inp$nindex
       == 1, "a relative abundance index", paste0(spict_dat$dataExplo$inp$nindex, " relative abundance indices"))`
       spanning from `r min(unlist(spict_dat$dataExplo$inp$timeI))` to `r max(unlist(spict_dat$dataExplo$inp$timeI))` (Figure 1).
...
The model `r if(spict_dat$results$opt$convergence == 0) "converged" else "did not converge, subsequent results should be interpreted with caution"`.
